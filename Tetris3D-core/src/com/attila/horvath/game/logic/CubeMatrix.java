...
public class CubeMatrix {
			...
			for (int j = 0; j < X; j++) {
				for (int k = 0; k < Z; k++) {
					if (referenceMatrix[k][i][j] == 1) {
						...
						cubeMatrix[k][i][j] = new Cube(model);
	...
	public boolean setCubeMatrix(int x, int y, int z, int[][][] objectMatrix, int actObj) {
		AssetsManager assets = new AssetsManager();
		Model model;
					...
					if ((y - j > 0) && ((x + i) >= 0 && (x + i) < X)
						...
						if (objectMatrix[i][j][k] == 1) {
							if ((y - Config.Y + j + 1) <= 1) {
								...
								Cube tempCube = new Cube(model);
								cubeZ = (x - gap) * 12 + (i * 12);
								cubeX = (z - gap) * 12 + (k * 12);
								cubeY = (y - Config.Y + j + 1) * -12;

								tempCube.transform.translate(-cubeX,
										cubeY - 12, cubeZ);
								tempCube.materials
										.get(0)
										.set(ColorAttribute
												.createDiffuse(Config.colors[actObj - 1]));
	...
	public int translateCubeMatrix(ArrayList<Integer> fullRowArray) {
		int index = 0;
		int moveStep = 1;
